...
// A modern SVG icon for the  :::>>> Component ::>>>>
const LockIcon = () => (
  <svg
...
const VerifyEmail = ({ onVerifySuccess, data }) => {
  ...
  useEffect(() => {
    if (data.verificationcode) {
      setVerificationcode(data.verificationcode); // updation:::>>>
    }
  }, [data.verificationcode]); // when the verification code of the imported variable changes 
  ...
  const handleVerify = async () => {
    ...
    try {
      if (code === verificationcode) {
        // now remove the token of user if have and give the token of admin
        localStorage.removeItem('loginToken')  // remove the token of user if have 
        localStorage.setItem('loginToken', token);  // adding the token of admin
